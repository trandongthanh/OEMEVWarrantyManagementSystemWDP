...
async function seedDatabase() {
  try {
    ...
    const [battVFe34] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Pin Cao Áp 42kWh (VF e34)", price: 280000000, sku: "BAT-HV-42KWH-VFE34", category: "HIGH_VOLTAGE_BATTERY" },
    });
    ...
    const [battVF8] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Pin Cao Áp 87.7kWh (VF 8)", price: 420000000, sku: "BAT-HV-87KWH-VF8", category: "HIGH_VOLTAGE_BATTERY" },
    });
    typeComponents.battVF8 = battVF8;

    const [battVF9] = await sequelize.models.TypeComponent.findOrCreate({
      where: { sku: "BAT-HV-123KWH-VF9" },
      defaults: { name: "Pin Cao Áp 123kWh (VF 9)", price: 550000000, sku: "BAT-HV-123KWH-VF9", category: "HIGH_VOLTAGE_BATTERY" },
    });
    typeComponents.battVF9 = battVF9;

    const [bms] = await sequelize.models.TypeComponent.findOrCreate({
      where: { sku: "BMS-CTRL-GEN3" },
      defaults: { name: "Bộ Quản Lý Pin BMS Gen3", price: 45000000, sku: "BMS-CTRL-GEN3", category: "HIGH_VOLTAGE_BATTERY" },
    });
    ...
    const [motor110] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Động Cơ Điện 110kW (VF e34, VF 5 Plus)", price: 150000000, sku: "MOT-ELC-110KW", category: "POWERTRAIN" },
    });
    ...
    const [motor150] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Động Cơ Điện 150kW RWD (VF 8)", price: 180000000, sku: "MOT-ELC-150KW-RWD", category: "POWERTRAIN" },
    });
    ...
    const [motor300] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Hệ Thống Động Cơ 300kW AWD (VF 9)", price: 350000000, sku: "MOT-ELC-300KW-AWD", category: "POWERTRAIN" },
    });
    typeComponents.motor300 = motor300;

    const [inverter] = await sequelize.models.TypeComponent.findOrCreate({
      where: { sku: "INV-PWR-400V" },
      defaults: { name: "Biến Tần Công Suất 400V", price: 85000000, sku: "INV-PWR-400V", category: "POWERTRAIN" },
    });
    ...
    const [onboardCharger] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Bộ Sạc Tích Hợp 11kW", price: 35000000, sku: "CHG-OBC-11KW", category: "CHARGING_SYSTEM" },
    });
    ...
    const [chargingPort] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Cổng Sạc CCS2", price: 8000000, sku: "CHG-PORT-CCS2", category: "CHARGING_SYSTEM" },
    });
    ...
    const [coolingSystem] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Hệ Thống Làm Mát Pin", price: 55000000, sku: "THRM-COOL-BATT", category: "THERMAL_MANAGEMENT" },
    });
    ...
    const [heatPump] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Bơm Nhiệt Hiệu Suất Cao", price: 42000000, sku: "THRM-HPUMP-EFF", category: "THERMAL_MANAGEMENT" },
    });
    ...
    const [battery12v] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Ắc Quy 12V AGM", price: 5000000, sku: "LV-BATT-12V-AGM", category: "LOW_VOLTAGE_SYSTEM" },
    });
    ...
    const [dcConverter] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Bộ Chuyển Đổi DC-DC", price: 18000000, sku: "LV-DCDC-CONV", category: "LOW_VOLTAGE_SYSTEM" },
    });
    ...
    const [brakingSystem] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Hệ Thống Phanh Tái Sinh + ABS", price: 65000000, sku: "BRK-REGEN-ABS", category: "BRAKING" },
    });
    ...
    const [brakePads] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Má Phanh Ceramic (Bộ 4)", price: 4500000, sku: "BRK-PAD-CERAMIC", category: "BRAKING" },
    });
    ...
    const [suspension] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Hệ Thống Treo Thích Ứng", price: 75000000, sku: "SUSP-ADAPT-DAMP", category: "SUSPENSION_STEERING" },
    });
    ...
    const [steering] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Hệ Thống Lái Trợ Lực Điện", price: 32000000, sku: "STEER-EPS-RACK", category: "SUSPENSION_STEERING" },
    });
    ...
    const [hvacSystem] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Điều Hòa Tự Động 2 Vùng", price: 38000000, sku: "HVAC-AUTO-DUAL", category: "HVAC" },
    });
    ...
    const [airFilter] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Bộ Lọc Không Khí HEPA", price: 3500000, sku: "HVAC-FILTER-HEPA", category: "HVAC" },
    });
    ...
    const [bodyPanel] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Tấm Thân Xe Phía Trước", price: 25000000, sku: "BODY-PANEL-FRONT", category: "BODY_CHASSIS" },
    });
    ...
    const [windshield] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Kính Chắn Gió Sưởi", price: 15000000, sku: "BODY-WSHIELD-HEAT", category: "BODY_CHASSIS" },
    });
    ...
    const [display10] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Màn Hình LCD 10 inch (VF e34, VF 5)", price: 15000000, sku: "INFO-LCD-10IN", category: "INFOTAINMENT_ADAS" },
    });
    ...
    const [display15] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Màn Hình LCD 15.6 inch (VF 8, VF 9)", price: 28000000, sku: "INFO-LCD-15IN", category: "INFOTAINMENT_ADAS" },
    });
    ...
    const [adasSystem] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Hệ Thống ADAS Cấp 2", price: 95000000, sku: "ADAS-LVL2-SUITE", category: "INFOTAINMENT_ADAS" },
    });
    ...
    const [camera360] = await sequelize.models.TypeComponent.findOrCreate({
      ...
      defaults: { name: "Camera 360 độ", price: 18000000, sku: "ADAS-CAM-360", category: "INFOTAINMENT_ADAS" },
    });
    ...
    for (const comp of [typeComponents.battVFe34, typeComponents.battVF8, typeComponents.battVF9, typeComponents.bms]) {
    ...
    for (const comp of [
      typeComponents.motor110, typeComponents.motor150, typeComponents.motor300,
      typeComponents.inverter, typeComponents.brakingSystem, typeComponents.steering
    ]) {
      await sequelize.models.TypeComponentByCompany.findOrCreate({
        where: {
    ...
    for (const comp of [
      typeComponents.display10, typeComponents.display15, 
      typeComponents.adasSystem, typeComponents.camera360, typeComponents.hvacSystem
    ]) {
      await sequelize.models.TypeComponentByCompany.findOrCreate({
        where: {
    ...
    const vehicleComponentMap = {
      vfe34: [
        { comp: typeComponents.battVFe34, qty: 1, duration: 96, mileage: 160000 },
        { comp: typeComponents.bms, qty: 1, duration: 96, mileage: 160000 },
        { comp: typeComponents.motor110, qty: 1, duration: 96, mileage: 160000 },
        { comp: typeComponents.inverter, qty: 1, duration: 96, mileage: 160000 },
        { comp: typeComponents.onboardCharger, qty: 1, duration: 60, mileage: 120000 },
        { comp: typeComponents.chargingPort, qty: 1, duration: 60, mileage: 120000 },
        { comp: typeComponents.coolingSystem, qty: 1, duration: 60, mileage: 120000 },
        { comp: typeComponents.battery12v, qty: 1, duration: 24, mileage: 50000 },
        { comp: typeComponents.dcConverter, qty: 1, duration: 60, mileage: 120000 },
        { comp: typeComponents.brakingSystem, qty: 1, duration: 60, mileage: 120000 },
        { comp: typeComponents.brakePads, qty: 1, duration: 12, mileage: 20000 },
        { comp: typeComponents.steering, qty: 1, duration: 60, mileage: 120000 },
        { comp: typeComponents.hvacSystem, qty: 1, duration: 36, mileage: 80000 },
        { comp: typeComponents.display10, qty: 1, duration: 36, mileage: 80000 },
        { comp: typeComponents.camera360, qty: 1, duration: 36, mileage: 80000 },
      ],
      vf8: [
        { comp: typeComponents.battVF8, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.bms, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.motor150, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.inverter, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.onboardCharger, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.chargingPort, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.coolingSystem, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.heatPump, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.battery12v, qty: 1, duration: 24, mileage: 50000 },
        { comp: typeComponents.dcConverter, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.brakingSystem, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.brakePads, qty: 1, duration: 12, mileage: 20000 },
        { comp: typeComponents.suspension, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.steering, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.hvacSystem, qty: 1, duration: 60, mileage: 120000 },
        { comp: typeComponents.display15, qty: 1, duration: 60, mileage: 120000 },
        { comp: typeComponents.adasSystem, qty: 1, duration: 60, mileage: 120000 },
        { comp: typeComponents.camera360, qty: 1, duration: 60, mileage: 120000 },
      ],
      vf9: [
        { comp: typeComponents.battVF9, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.bms, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.motor300, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.inverter, qty: 2, duration: 120, mileage: 200000 }, // AWD có 2 inverter
        { comp: typeComponents.onboardCharger, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.chargingPort, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.coolingSystem, qty: 1, duration: 120, mileage: 200000 },
        { comp: typeComponents.heatPump, qty: 1, duration: 120, mileage: 200000 },
      ...
      vf5Plus: [
        { comp: typeComponents.battVFe34, qty: 1, duration: 84, mileage: 160000 }, // Dùng chung pin với e34
        { comp: typeComponents.bms, qty: 1, duration: 84, mileage: 160000 },
        { comp: typeComponents.motor110, qty: 1, duration: 84, mileage: 160000 },
        { comp: typeComponents.inverter, qty: 1, duration: 84, mileage: 160000 },
        { comp: typeComponents.onboardCharger, qty: 1, duration: 84, mileage: 160000 },
        { comp: typeComponents.chargingPort, qty: 1, duration: 84, mileage: 160000 },
        { comp: typeComponents.coolingSystem, qty: 1, duration: 84, mileage: 160000 },
        { comp: typeComponents.battery12v, qty: 1, duration: 24, mileage: 50000 },
        { comp: typeComponents.dcConverter, qty: 1, duration: 84, mileage: 160000 },
        { comp: typeComponents.brakingSystem, qty: 1, duration: 84, mileage: 160000 },
        { comp: typeComponents.brakePads, qty: 1, duration: 12, mileage: 20000 },
        { comp: typeComponents.steering, qty: 1, duration: 84, mileage: 160000 },
        { comp: typeComponents.hvacSystem, qty: 1, duration: 48, mileage: 100000 },
        { comp: typeComponents.display10, qty: 1, duration: 48, mileage: 100000 },
      ],
    };
    ...
    for (const [modelKey, components] of Object.entries(vehicleComponentMap)) {
      ...
      console.log(`  ✓ ${vehicleModels[modelKey].vehicleModelName}: ${components.length} components`);
    }
    ...
    // HÀ NỘI
    const hnUsers = [
      { username: "staff_hn1", name: "Nguyễn Văn An", role: "service_center_staff", sc: scHN },
      { username: "staff_hn2", name: "Đỗ Thị Mai", role: "service_center_staff", sc: scHN },
      { username: "staff_hn3", name: "Bùi Văn Hùng", role: "service_center_staff", sc: scHN },
      { username: "tech_hn1", name: "Lê Văn Cường", role: "service_center_technician", sc: scHN },
      { username: "tech_hn2", name: "Vũ Minh Tuấn", role: "service_center_technician", sc: scHN },
      { username: "tech_hn3", name: "Ngô Thanh Long", role: "service_center_technician", sc: scHN },
      { username: "tech_hn4", name: "Đinh Văn Nam", role: "service_center_technician", sc: scHN },
      { username: "manager_hn", name: "Trần Thị Bình", role: "service_center_manager", sc: scHN },
      { username: "parts_sc_hn1", name: "Hoàng Thị Em", role: "parts_coordinator_service_center", sc: scHN },
      { username: "parts_sc_hn2", name: "Lý Văn Tâm", role: "parts_coordinator_service_center", sc: scHN },
    ];

    // TP.HCM
    const hcmUsers = [
      { username: "staff_hcm1", name: "Võ Văn Khoa", role: "service_center_staff", sc: scHCM },
      { username: "staff_hcm2", name: "Phan Thị Lan", role: "service_center_staff", sc: scHCM },
      { username: "tech_hcm1", name: "Trương Văn Phong", role: "service_center_technician", sc: scHCM },
      { username: "tech_hcm2", name: "Huỳnh Văn Tài", role: "service_center_technician", sc: scHCM },
      { username: "tech_hcm3", name: "Lâm Thị Hoa", role: "service_center_technician", sc: scHCM },
      { username: "manager_hcm", name: "Nguyễn Thị Xuân", role: "service_center_manager", sc: scHCM },
      { username: "parts_sc_hcm1", name: "Đặng Văn Minh", role: "parts_coordinator_service_center", sc: scHCM },
    ];

    // CÔNG TY
    const companyUsers = [
      { username: "emv_staff1", name: "Phạm Văn Dũng", role: "emv_staff", company: vehicleCompany },
      { username: "emv_staff2", name: "Lê Thị Nga", role: "emv_staff", company: vehicleCompany },
      { username: "parts_company1", name: "Đặng Văn Phúc", role: "parts_coordinator_company", company: vehicleCompany },
      { username: "parts_company2", name: "Cao Văn Sơn", role: "parts_coordinator_company", company: vehicleCompany },
      { username: "admin", name: "Võ Thị Giang (Admin)", role: "emv_admin", company: vehicleCompany },
    ];
    ...
    for (const userData of [...hnUsers, ...hcmUsers, ...companyUsers]) {
      const [user] = await sequelize.models.User.findOrCreate({
        ...
        defaults: {
          ...
          address: userData.sc ? (userData.sc === scHN ? "Hà Nội" : "TP.HCM") : "Hà Nội",
          name: userData.name,
          roleId: roles[userData.role].roleId,
          serviceCenterId: userData.sc ? userData.sc.serviceCenterId : null,
          vehicleCompanyId: userData.company ? userData.company.vehicleCompanyId : null,
        },
      });
      allUsers.push(user);
    }

    console.log(`✅ User: ${allUsers.length} người dùng (đầy đủ 7 vai trò cho 2 trung tâm)`);

    // ========================================
    // 11. TẠO WORK SCHEDULE CHO KỸ THUẬT VIÊN
    // ========================================
    const allTechs = allUsers.filter(u => u.name.includes("Cường") || u.name.includes("Tuấn") || u.name.includes("Long") || u.name.includes("Nam") || u.name.includes("Phong") || u.name.includes("Tài") || u.name.includes("Hoa"));
    ...
    console.log(`✅ WorkSchedule: ${allTechs.length * 30} lịch (${allTechs.length} kỹ thuật viên x 30 ngày)`);
    ...
    const customers = [];
    const customerData = [
      { fullName: "Nguyễn Văn An", phone: "0901111111", email: "nguyenvanan@gmail.com", address: "123 Đường ABC, Hà Nội" },
      { fullName: "Trần Thị Bình", phone: "0902222222", email: "tranthibinh@gmail.com", address: "456 Đường DEF, TP.HCM" },
      { fullName: "Lê Hoàng Cường", phone: "0903333333", email: "lehoangcuong@gmail.com", address: "789 Đường GHI, Đà Nẵng" },
      { fullName: "Phạm Minh Đức", phone: "0904444444", email: "phamminhduc@gmail.com", address: "321 Đường JKL, Hải Phòng" },
      { fullName: "Hoàng Thu Hà", phone: "0905555555", email: "hoangthuha@gmail.com", address: "654 Đường MNO, Cần Thơ" },
    ];
    ...
    const vehiclesData = [
      // VF e34 - 4 xe
      { model: vehicleModels.vfe34, vin: "VFE34-2023-001", plate: "30A-12345", owner: customers[0], purchaseDate: "2023-06-01", mfgDate: "2023-05-15" },
      { model: vehicleModels.vfe34, vin: "VFE34-2023-002", plate: "30A-12346", owner: customers[0], purchaseDate: "2023-08-15", mfgDate: "2023-08-01" },
      { model: vehicleModels.vfe34, vin: "VFE34-2023-003", plate: "51F-23456", owner: customers[1], purchaseDate: "2023-07-10", mfgDate: "2023-06-25" },
      { model: vehicleModels.vfe34, vin: "VFE34-2023-004", plate: "43A-34567", owner: customers[2], purchaseDate: "2023-05-15", mfgDate: "2023-05-01" },
      
      // VF 8 - 3 xe
      { model: vehicleModels.vf8, vin: "VF8-2023-001", plate: "51F-88888", owner: customers[1], purchaseDate: "2023-09-20", mfgDate: "2023-09-05" },
      { model: vehicleModels.vf8, vin: "VF8-2023-002", plate: "16B-45678", owner: customers[3], purchaseDate: "2023-06-20", mfgDate: "2023-06-05" },
      { model: vehicleModels.vf8, vin: "VF8-2024-003", plate: "65C-56789", owner: customers[4], purchaseDate: "2024-04-05", mfgDate: "2024-03-20" },
      
      // VF 9 - 2 xe
      { model: vehicleModels.vf9, vin: "VF9-2024-001", plate: "30A-99999", owner: customers[2], purchaseDate: "2024-10-01", mfgDate: "2024-09-15" },
      { model: vehicleModels.vf9, vin: "VF9-2024-002", plate: "51F-99999", owner: customers[3], purchaseDate: "2024-11-10", mfgDate: "2024-10-25" },
      
      // VF 5 Plus - 3 xe
      { model: vehicleModels.vf5Plus, vin: "VF5P-2024-001", plate: "30A-55555", owner: customers[0], purchaseDate: "2024-12-01", mfgDate: "2024-11-15" },
      { model: vehicleModels.vf5Plus, vin: "VF5P-2024-002", plate: "51F-55555", owner: customers[4], purchaseDate: "2024-08-20", mfgDate: "2024-08-05" },
      { model: vehicleModels.vf5Plus, vin: "VF5P-2024-003", plate: "16B-55555", owner: customers[2], purchaseDate: "2024-07-15", mfgDate: "2024-07-01" },
    ];
    ...
    for (const vehData of vehiclesData) {
      ...
      createdVehicles.push({ vehicle, modelKey: Object.keys(vehicleModels).find(k => vehicleModels[k].vehicleModelId === vehData.model.vehicleModelId) });
    }
    ...
    for (const whConfig of stockConfig) {
      ...
      for (const compConfig of whConfig.comps) {
        ...
        for (let i = 1; i <= compConfig.qty; i++) {
          const serial = `${compConfig.type.sku}-${whConfig.wh.name.substring(0, 4).toUpperCase()}-${String(i).padStart(5, "0")}`;
        ...
        console.log(`    ✓ ${compConfig.type.name}: ${compConfig.qty} components`);
      }
    }
    ...
    for (const { vehicle, modelKey } of createdVehicles) {
      ...
      console.log(`  ✓ ${vehicle.licensePlate} (${vehicle.vin}): ${componentsForModel.reduce((sum, c) => sum + c.qty, 0)} components`);
    }
    ...
    console.log("   🏭 3 Nhà cung cấp: CATL Battery, Bosch Automotive, LG Electronics");
    ...
    console.log(`   ⚙️  ${totalStockComponents + totalInstalledComponents} Components:`);
    console.log(`      - ${totalStockComponents} trong kho (status: IN_WAREHOUSE)`);
    console.log(`      - ${totalInstalledComponents} đã lắp trên xe (status: INSTALLED)`);
    ...
    console.log(`   📅 ${allTechs.length * 30} Lịch làm việc (${allTechs.length} kỹ thuật viên x 30 ngày)`);

    console.log("\n🔑 TÀI KHOẢN TEST (password: 123456):");
    console.log("   👔 Staff:       staff_hn1, staff_hn2, staff_hn3, staff_hcm1, staff_hcm2");
    ...
    console.log("\n💡 QUAN TRỌNG:");
    console.log("   ✓ Stock.quantityInStock = SỐ COMPONENT THẬT SỰ có status IN_WAREHOUSE");
    console.log("   ✓ Mỗi xe có ĐẦY ĐỦ components theo WarrantyComponent của model đó");
    console.log("   ✓ Component INSTALLED: status='INSTALLED', vehicleVin=<vin>, warehouseId=null");
    console.log("   ✓ Component IN_WAREHOUSE: status='IN_WAREHOUSE', vehicleVin=null, warehouseId=<id>");
    console.log("   ✓ WarrantyComponent định nghĩa component NÀO được phép lắp vào model xe NÀO");
    console.log("=".repeat(80) + "\n");
